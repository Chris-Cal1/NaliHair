...
// acquisition des articles liké de la db
  useEffect(() => {
    const findArticlesWishList = async () => {
      ...
      const dataWishlist = await fetch(`http://10.0.0.103:3000/wishlist-articles?token=${props.token}`)
...
// suppression d'un article
  ...
  const response = await fetch('http://10.0.0.103:3000/delete-article', {
   method: 'DELETE',
...
// recherche d'un article
  var findArticle = async () => {
    ...
    const saveReq = await fetch('http://10.0.0.103:3000/wishlist-article', {
